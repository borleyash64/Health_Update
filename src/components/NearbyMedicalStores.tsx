...
import { Pill, Loader2, AlertTriangle, ExternalLink, Star } from 'lucide-react';
...
export function NearbyMedicalStores() {
      ...
      async (position) => {
        try {
          ...
          const response = await ai.models.generateContent({
            ...
            contents: 'What are the best pharmacies, chemists, or medical stores nearby? Please provide a brief overview of the top options.',
            config: {
              tools: [{ googleMaps: {} }],
              toolConfig: {
                retrievalConfig: {
          ...
          setError(err.message || 'An error occurred while fetching nearby medical stores.');
          console.error(err);
        } finally {
          setIsLoading(false);
        }
    ...
    <div className="space-y-6">
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-sm border border-white p-6 md:p-8">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div>
            <h2 className="text-2xl font-semibold text-slate-900 tracking-tight">Find Nearby Medical Stores</h2>
          ...
          <button
            ...
            disabled={isLoading}
            className="mt-4 inline-flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
          >
            {isLoading ? (
              <>
              ...
              <div className="space-y-3">
                {places.map((place, index) => (
                  <a
                    ...
                    className="block bg-white/80 backdrop-blur-sm rounded-2xl border border-white shadow-sm p-5 hover:border-emerald-300 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group"
                  >
